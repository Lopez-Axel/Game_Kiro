...
func move_to(target_position: Vector2) -> void:
	...
	pose.play(_get_direction_anim(move_direction))
	animation_playback.start(&"walk")

	var tween := create_tween()
	tween.set_ease(Tween.EASE_IN)
	var end: Vector2 = $Pivot.position + move_direction * grid_size
	tween.tween_property($Pivot, ^"position", end, walk_animation_time / speed_multiplier)
...
func _get_direction_anim(direction: Vector2) -> StringName:
	if direction.x < 0:
		return &"left"
	elif direction.x > 0:
		return &"right"
